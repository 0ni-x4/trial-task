...
interface DesktopEditorSectionProps {
  essayContent: string;
...
export const DesktopEditorSection = ({
  essayContent,
  onContentChange,
  highlights,
  currentIndex,
  ...
  wordCount,
  maxWords,
}: DesktopEditorSectionProps) => {
  return (
    <div className="lg:col-span-2 flex flex-col min-h-0">
      {/* Toolbar */}
      <div className="flex items-center justify-between bg-card rounded-t-[20px] px-4 py-2 border border-border border-b-0">
        <div className="flex gap-4 text-muted-foreground">
          ...
          <div
            ...
            title={`${saveStatus === 'saving' ? 'Saving...' : saveStatus === 'saved' ? 'Saved' : 'Unsaved changes'} (Ctrl/Cmd+S)`}
          >
            {saveStatus === 'saving' ? (
