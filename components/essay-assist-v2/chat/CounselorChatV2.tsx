...
// Global state management to prevent reset on tab switches
interface ChatState {
  messages: Message[];
  input: string;
  isTyping: boolean;
  expandedHighlights: { [key: number]: boolean };
}

const chatStates = new Map<string, ChatState>();

const getChatStateKey = (initialMessages?: Array<any>) => {
  // Create a unique key based on the initial messages length and first message content
  return `chat_${initialMessages?.length || 0}_${initialMessages?.[0]?.content?.slice(0, 50) || 'empty'}`;
};

export const CounselorChatV2: FC<CounselorChatProps> = ({
  essay,
  suggestions,
  onHighlight,
  initialMessages,
  ...
  const chatKey = getChatStateKey(initialMessages);
  const scrollAreaRef = useRef<HTMLDivElement>(null);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Initialize or get existing state
  const initState = () => {
    if (chatStates.has(chatKey)) {
      return chatStates.get(chatKey)!;
    }
    ...
    chatStates.set(chatKey, initialState);
    return initialState;
  };
  ...
  const updateState = (updates: Partial<ChatState> | ((prev: ChatState) => Partial<ChatState>)) => {
    setState(prevState => {
      const updatesObj = typeof updates === 'function' ? updates(prevState) : updates;
      const newState = { ...prevState, ...updatesObj };
      chatStates.set(chatKey, newState);
      return newState;
    });
  };

  // Initialize messages from database (only once per chat session)
  useEffect(() => {
    if (initialMessages && initialMessages.length > 0) {
      const currentState = chatStates.get(chatKey);
      // Only initialize if we haven't loaded these messages before
  ...
  const sendQuestion = async (question: string) => {
    ...
    try {
      ...
      // Set highlights immediately when response comes back
      if (onHighlight) {
  ...
  const toggleHighlights = (messageIndex: number) => {
    updateState(prevState => ({
      expandedHighlights: {
        ...prevState.expandedHighlights,
        [messageIndex]: !prevState.expandedHighlights[messageIndex],
      }
    }));
  };

  const collapsed = state.messages.length >= 2;
  ...
  return (
    <Card className="p-2 rounded-[20px] flex flex-col flex-1 min-h-0 overflow-hidden">
      ...
      {/* Chat messages */}
      {state.messages.length > 0 && (
        <ScrollArea ref={scrollAreaRef} className="flex-1 mb-4 pr-3 rounded-[5px]">
                ...
                return (
                  <div key={i} className="text-left">
                    <div className="inline-block bg-muted px-2 py-2 rounded-lg text-sm max-w-[85%]">
                      {/* Highlighted text dropdown */}
                      {m.highlights && m.highlights.length > 0 && (
                        <div className="mb-2">
                          <button
                            onClick={() => toggleHighlights(i)}
                            className="inline-flex bg-popover items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors px-2 py-1 rounded-md hover:bg-background/50 border border-border w-full justify-between"
                          >
                            <div className="flex items-center gap-1">
                              <Highlighter className="w-3 h-3 text-teal-500" />
                              Highlighted text ({m.highlights.length})
                            </div>
                            {state.expandedHighlights[i] ? (
      ...
      {state.messages.length === 0 && <div className="flex-1" />}
